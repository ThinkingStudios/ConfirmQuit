        ...
        ButtonPressEvent.BUTTON_PRESS.register((button) -> {
            ...
            if ("menu.returnToMenu".equals(key)) {
                ...
                if (ConfigHelper.getConfig().confirmTypeInSingleplayer == ConfirmType.SCREEN) {
                    MinecraftClient.getInstance().setScreen(new ConfirmScreen(MinecraftClient.getInstance().currentScreen, Text.translatable("screen.rusurequit.confirm.content.insingleplay"), () -> button.onPress(new MouseInput(InputUtil.GLFW_MOUSE_BUTTON_LEFT, 0))));
                    return EventResult.CANCEL;
                }
                return EventResult.PASS;
            }
            if ("menu.disconnect".equals(key)) {
                ...
                if (ConfigHelper.getConfig().confirmTypeInMultiplayer == ConfirmType.SCREEN) {
                    MinecraftClient.getInstance().setScreen(new ConfirmScreen(MinecraftClient.getInstance().currentScreen, Text.translatable("screen.rusurequit.confirm.content.inmultiplay"), () -> button.onPress(new MouseInput(InputUtil.GLFW_MOUSE_BUTTON_LEFT, 0))));
                    return EventResult.CANCEL;
                }
                return EventResult.PASS;
            }
